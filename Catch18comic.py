...
待办事宜：
当前mkIndex还是根据comic_num计数在生成的html中显示图片。应该及早更新为imgs数列
'''
public_headers = {
...
SKIP_EXIST = "1" # 跳过已经下载的章节。在photo下载时根据是否存在index.html（上次成功全集下载）判断是否跳过该章节。因为该数字为输入，所以直接字符串
...
def convertImg(img_url):
        ...
        for img_count in range(9):  #处理前9个规则高度的块
            img_crop_box = (0, img_crop_size*img_count, img_width, img_crop_size*(img_count+1))
            img_crop_area = img.crop(img_crop_box)
            img_crop_yposition = img_size[1] - img_crop_size * (img_count+1)
            img_block_list.append((img_crop_area,img_crop_yposition)) #存为(切前范围、未来y位置)的元组
        img_block_list.append((img.crop((0, img_crop_size*9, img_width, img_size[1])), 0))  #单独处理末尾异常高度块
        img_new = Image.new('RGB', (img_size[0], img_size[1]))
        for img_block,yposition in img_block_list : 
            img_new.paste(img_block, (0, yposition)) 
...
def get_url_list2(url): #原get_url_list方法采用编号推算，对编号断层没有办法处理，这是新的试验办法
    ...
    for page in rawList :
        if page.find("jpg") >= 0 :   #有些图片是空白图片，例如名字不是"xxxxx.jpg"而是"extract",这会产生无效的下载地址
...
def main(mirror, id):
    ...
    # 判断是否跳过已下载章节
    if SKIP_EXIST == "1" and os.path.exists(path + "/index.html"):
        print('该章节已经下载，跳过……')
    else :
        #开始下载操作
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if flag: #有就进行解析，无就直接下载
            check_all_download = input('Tips:检测到您输入的链接是包括多个章节的，请判断是否自动跳过已经下载的章节：\n输入数字0:重新下载全部章节\t输入数字1:自动跳过已经下载的章节\n')
            if check_all_download == '1' or check_all_download == '0':
                SKIP_EXIST = check_all_download
